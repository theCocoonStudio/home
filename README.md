...
By removing the hierarchy required -- i.e., nesting `RenderTextures`, you can put your `RenderTexture` instances (wrapped by `ShaderPass`) anywhere in the `ShaderPassesTexture` tree. This removes the one-way relationship of the FBO input-output chain.
