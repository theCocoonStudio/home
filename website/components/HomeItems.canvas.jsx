  ...
  const {
    ...
    if (modelsSize) {
      ...
      for (let i = 0; i < itemsConfig.length; i++) {
        ...
        const markup = document.getElementById(`${itemDescriptionIdBase}-${i}`)
      ...
      const focusScalesFactor =
        isCenterLayout || isMobileLayout ? factorCenterLayout : modelsFactor
    ...
    for (let index = 0; index < itemsConfig.length; index++) {
      returnItems[index] = (
        <HomeItem
          ...
          focusScalesFactor={focusScalesFactor}
